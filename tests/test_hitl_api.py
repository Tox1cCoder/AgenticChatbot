...
class _CatalogSession:
    def __init__(self, user_id, tools):
        ...
        self.tool_catalog = {"tools": tools}


def test_get_settings_with_device_id_annotates_rule_availability(api, monkeypatch):
    client, user_id, _sf = api
    client.post(
        "/hitl/settings",
        json={
            "items": [
                {
                    "scopeType": "server",
                    "scopeValue": "skill_cli_anything_google_calendar",
    ...
    monkeypatch.setattr(
        "app.services.hitl_settings_service._lookup_device_session",
        lambda requested_user_id, requested_device_id: (
            session if requested_device_id == device_id else None
        ),
    )

    data = client.get(f"/hitl/settings?deviceId={device_id}").json()["data"]
    servers = {s["scopeValue"]: s for s in data["servers"]}
    tools = {t["scopeValue"]: t for t in data["tools"]}

    assert servers["desktop_commander"]["available"] is True
    assert servers["skill_cli_anything_google_calendar"]["available"] is False
    assert tools["skill::cli-anything-google-calendar::run_skill_command"]["available"] is False


def test_get_settings_without_device_id_omits_availability(api):
    client, _user_id, _sf = api
    client.post(
        "/hitl/settings",
        json={
    ...
    for rule in data["servers"] + data["tools"]:
        assert "available" not in rule


def test_delete_accepts_camel_case_scope_params(api):
    client, _user_id, _sf = api
    client.post(
        "/hitl/settings",
        json={
    ...
    resp = client.request(
        "DELETE", "/hitl/settings", params={"scopeType": "server", "scopeValue": "excel"}
    )
    assert resp.status_code == 200
    data = client.get("/hitl/settings").json()["data"]
