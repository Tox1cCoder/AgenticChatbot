...
**Task 3 (base_agent.py, token_instrumentation.py):** Implemented as planned. `image_context` import placed alphabetically after `hand_off_tool`. History conversion only upgrades a user message to multimodal when `has_image_parts` is true, otherwise keeps the plain string (avoids wrapping text-only history in list form unnecessarily). Token estimator adds a flat 32-token-per-attachment overhead for trimming — an estimate, not a cap. 2/2 tests pass. Committed as `9a969bf`.
