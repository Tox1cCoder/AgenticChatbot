...
down_revision: Union[str, None] = '1ce64a959f7d'
...
def upgrade() -> None:
    ...
    sa.Column('decision', sa.Enum('ACCEPT', 'EDIT', 'REJECT', name='decision_type'), nullable=False),
    sa.Column('decided_at', sa.DateTime(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['conversation_id'], ['conversations.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_tool_approvals_conversation_id'), 'tool_approvals', ['conversation_id'], unique=False)
    op.create_index(op.f('ix_tool_approvals_decided_at'), 'tool_approvals', ['decided_at'], unique=False)
    op.create_index(op.f('ix_tool_approvals_id'), 'tool_approvals', ['id'], unique=False)
    op.create_index(op.f('ix_tool_approvals_interrupt_id'), 'tool_approvals', ['interrupt_id'], unique=False)
    ...
    op.create_index(op.f('ix_hitl_interrupts_assistant_message_id'), 'hitl_interrupts', ['assistant_message_id'], unique=False)
    ...
    op.create_index('idx_task_plan_conversation_order', 'task_plans', ['conversation_id', 'task_order'], unique=False)
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('task_plans', sa.Column('actual_duration_minutes', sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column('task_plans', sa.Column('retry_count', sa.INTEGER(), server_default=sa.text('0'), autoincrement=False, nullable=False))
    op.add_column('task_plans', sa.Column('completion_confidence', sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=True))
    op.add_column('task_plans', sa.Column('started_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True))
    op.add_column('task_plans', sa.Column('estimated_duration_minutes', sa.INTEGER(), autoincrement=False, nullable=True))
    ...
    op.create_index(op.f('ix_task_plans_conversation_order'), 'task_plans', ['conversation_id', 'task_order'], unique=False)
    ...
    op.add_column('messages', sa.Column('citations', postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True))
    op.add_column('messages', sa.Column('extra_metadata', postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True))
    ...
    op.create_index(op.f('ix_hitl_interrupts_status_expires_at'), 'hitl_interrupts', ['status', 'expires_at'], unique=False)
    ...
    sa.Column('token_type', sa.VARCHAR(length=50), server_default=sa.text("'Bearer'::character varying"), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('mcp_oauth_tokens_user_id_fkey'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('mcp_oauth_tokens_pkey')),
    sa.UniqueConstraint('user_id', 'server_name', name=op.f('uq_mcp_oauth_user_server'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    ...
    op.create_index(op.f('ix_mcp_oauth_tokens_server_name'), 'mcp_oauth_tokens', ['server_name'], unique=False)
