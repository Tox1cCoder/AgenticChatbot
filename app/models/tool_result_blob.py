...
class ToolResultBlob(Base):
    ...
    The full payload lives on disk under ``storage_path`` (relative to the
    configured storage root). The model only sees a preview plus a blob_id
    pointer it can use later to read the full result.
    """
