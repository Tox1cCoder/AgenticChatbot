...
def log_token_breakdown(
    ...
    """
    Log token breakdown for observability.

    Args:
        breakdown: The computed token breakdown
        agent_id: ID of the agent making the request
        conversation_id: Optional conversation ID for context
        level: Logging level (default: DEBUG)
    """
    # Format tool names for logging (limit to avoid huge log lines)
    tool_names_display = breakdown.bound_tool_names[:10]
    if len(breakdown.bound_tool_names) > 10:
        tool_names_display.append(
            f"... and {len(breakdown.bound_tool_names) - 10} more"
        )

    logger.log(
        level,
        "Token breakdown for %s [conv=%s]: "
        "system=%d, history=%d (%d msgs), current=%d, "
        "tool_messages=%d, tool_schemas=%d (%d tools), "
        "TOTAL=%d | tools=%s",
        agent_id,
        conversation_id or "N/A",
        breakdown.system_prompt_tokens,
        breakdown.history_tokens,
        breakdown.history_message_count,
        breakdown.current_turn_tokens,
        breakdown.tool_message_tokens,
        breakdown.tool_schema_tokens,
        breakdown.bound_tool_count,
        breakdown.total_tokens,
        tool_names_display,
    )

    # Log actual usage if available
    if breakdown.actual_input_tokens is not None:
        logger.log(
            level,
            "Actual usage for %s: input=%d, output=%d",
            agent_id,
            breakdown.actual_input_tokens,
            breakdown.actual_output_tokens or 0,
        )


class TokenBudgetExceededWarning:
    """Helper to warn when token budgets are exceeded."""

    # Default budget thresholds
    DEFAULT_HISTORY_TOKEN_BUDGET = 8000
    DEFAULT_TOOL_RESULT_TOKEN_BUDGET = 4000
    DEFAULT_TOTAL_TOKEN_BUDGET = 20000
    ...
    def check_and_warn(
        ...
        tool_result_budget: Optional[int] = None,
        ...
        history_budget = history_budget or cls.DEFAULT_HISTORY_TOKEN_BUDGET
        tool_result_budget = tool_result_budget or cls.DEFAULT_TOOL_RESULT_TOKEN_BUDGET
        total_budget = total_budget or cls.DEFAULT_TOTAL_TOKEN_BUDGET

        if breakdown.history_tokens > history_budget:
            msg = (
                f"{agent_id}: History tokens ({breakdown.history_tokens}) "
                f"exceed budget ({history_budget})"
            )
            warnings.append(msg)
            logger.warning(msg)

        if breakdown.tool_message_tokens > tool_result_budget:
            msg = (
                f"{agent_id}: Tool message tokens ({breakdown.tool_message_tokens}) "
                f"exceed budget ({tool_result_budget})"
            )
            warnings.append(msg)
            logger.warning(msg)

        if breakdown.total_tokens > total_budget:
            msg = (
                f"{agent_id}: Total tokens ({breakdown.total_tokens}) "
                f"exceed budget ({total_budget})"
            )
            warnings.append(msg)
            logger.warning(msg)
