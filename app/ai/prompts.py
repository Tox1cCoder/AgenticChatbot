...
def build_rag_prompt(
    ...
    from ..utils.text_processing import estimate_tokens, truncate_text
    import logging
    ...
    if retrieved_docs:
        ...
        max_chunks = settings.rag_chunks_in_prompt if settings.rag_chunks_in_prompt > 0 else len(retrieved_docs)
        ...
        for i, doc in enumerate(retrieved_docs[:max_chunks], 1):
            ...
            if total_tokens + chunk_tokens > settings.rag_max_context_tokens and chunks_used >= 3:
                # We have at least 3 chunks and would exceed limit, stop here
                logger.debug(f"Stopping at {chunks_used} chunks to stay within token limit")
                break
            ...
            if total_tokens + chunk_tokens > settings.rag_max_context_tokens and chunks_used < 3:
                ...
                chunk_tokens = estimate_tokens(content)
                logger.debug(f"Truncated chunk {i} from {original_length} to {len(content)} chars to fit token limit")
            
            # Apply per-chunk character limit if configured
            if settings.max_chunk_chars_in_prompt > 0 and len(content) > settings.max_chunk_chars_in_prompt:
                content = truncate_text(content, settings.max_chunk_chars_in_prompt, add_ellipsis=True)
