...
_DESCRIPTION = (
    ...
    "make the visual subject more precise than the factual query. It reaches the "
    "image provider exactly as written, so write it as the search you would type "
    "yourself:\n"
    ...
    "- Resolve what the user referred to. 'my scooter', 'this game', 'it' cannot "
    "be searched — recover the named thing from the conversation first.\n"
    "- Write it in the language the subject is documented in, which is not "
    "necessarily the language of the answer. A product, place or interface known "
    "mainly in one country is photographed and captioned there; a global "
    "subject is best served in English however the user writes.\n"
    "- Name the form the question calls for — asked what a thing is, its "
    "identity image (logo, key art, cover, poster); asked how it works or looks "
    "in use, a photo, screenshot, diagram, map or chart.\n"
    "- Add the year or version when what matters is how the subject looks now: "
    "the image provider has no recency filter, so the query text is the only way "
    "to ask for a current picture.\n"
    "- No question words, and no disambiguator the search itself does not need.\n\n"
