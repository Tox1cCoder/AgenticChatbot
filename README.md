...
| **Bounded execution** | Model and tool call limits are enforced by framework middleware and end the turn with a typed `agent_execution_limit`. A task that needs more than one round of work is decomposed by Planning rather than silently restarted. |
...
Migrations are Alembic-managed (single head, currently `b8c9d0e1f2a3`). They are applied **automatically** at application startup via `app.database.migrations.upgrade_database` inside the lifespan hook, so manual migration is only required for dev or out-of-process tooling:
...
6. **Execution limits** — `ModelCallLimitMiddleware` and `ToolCallLimitMiddleware` bound the loop and raise, which the specialist wrapper turns into a typed `agent_execution_limit` failure. There is no outer continuation loop: a second round would re-enter `route`, and the turn's routing decision is set-once.
