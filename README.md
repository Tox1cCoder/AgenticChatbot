...
| **Bounded execution** | A per-epoch soft budget reserves one tool-free model call, so a turn that runs long answers from the evidence it has instead of being cut off. The framework's own ceiling sits above it as a last resort and produces a server-owned partial, never a generic execution error. A delegated Planning worker that exhausts its budget reports `partial` to its parent; only the top-level turn pauses for a Continue decision. |
...
Migrations are Alembic-managed (single head, currently `a3b4c5d6e7f9`). They are applied **automatically** at application startup via `app.database.migrations.upgrade_database` inside the lifespan hook, so manual migration is only required for dev or out-of-process tooling:
...
3. **Router** — [`RoutingService`](app/ai/workflow/routing.py) resolves the configured router model through the runtime model abstraction (no provider SDK of its own, no fallback) and returns a schema-constrained `RoutingDecision` naming one of `chat_agent` / `rag_agent` / `search_agent` / `image_generator_agent` / `planning_agent` / `canvas_agent` or an attached custom agent. `START` connects directly to `route`, which runs exactly once per turn.
...
6. **Execution limits** — a shared, framework-free accountant ([`execution_budget.py`](app/ai/workflow/execution_budget.py)) counts model and tool calls per epoch on the same path the specialist, RAG, and delegated-worker loops all take. Crossing the soft rung refuses the pending tool call with a paired `ToolMessage` and suppresses tools at the request boundary, so the next model call answers from the evidence already gathered. `ModelCallLimitMiddleware` and `ToolCallLimitMiddleware` remain above it as a last resort; their exception becomes a server-owned partial carrying the recorded artifacts, not an `agent_execution_limit` shown to the client. Re-entering `route` is still forbidden — a validated partial pauses at a control interrupt and Continue resumes that exact checkpoint with the previous epoch's evidence carried forward, so the turn's routing decision stays set-once.
...
**Global default tools.** Enabled servers in [`app/ai/mcp_config.json`](app/ai/mcp_config.json) are part of the global server catalog (currently `time`, `tavily`, `widgets`, and `brave_image_search`). Raw Tavily and Brave tools are excluded from ordinary model binding. Chat and search agents receive the canonical `web_search` and `web_open` product tools; their turn-scoped session owns provider fallback, source IDs, limits, image validation, and grounding. Machine-specific servers belong to the sidecar schema-v2 profile at `<profile>/<server-hash>/<user-id>/devices/<device-identifier>/mcp/config.v2.json`; credentials are stored separately in encrypted form.

`tavily` is one provider server with multiple retrieval tools. The product session resolves `tavily_search` and `tavily_extract` internally; the answer model sees only normalized, bounded source evidence with stable `S#` identifiers.
