...
| 2026-09-03 | Pre-existing schema drift (out of scope) | Recorded | `alembic check` still reports 4 items unrelated to this plan: `agent_model_configs.reasoning_effort` VARCHAR(32) vs Text, and three indexes present in the live DB but absent from the models (`ix_chat_images_sha256`, `idx_document_chunks_content_simple_fts`, `idx_document_chunks_index_generation_id`). None are touched by this refactor. Task 9 Step 1 expects `alembic check` clean and will not pass until these are reconciled separately. |
| 2026-09-03 | Integration-test target caveat | Recorded | The 8 receipt PostgreSQL tests seed and delete only their own `uuid4` rows, so they are safe against any reachable database. Two *pre-existing* integration modules are not: `test_model_usage_repository_postgres.py` and `test_conversation_compaction_postgres.py` assert over aggregates that include every row in the database, so they fail (5 failed, 1 error) when `TEST_DATABASE_URL` points at a populated database. Verified identical at `3380a72` with no local changes — not caused by this refactor. `TEST_DATABASE_URL` must name a dedicated test database, not the app database. |
