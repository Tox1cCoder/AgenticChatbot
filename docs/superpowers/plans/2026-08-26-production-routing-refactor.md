...
| 2026-09-03 | Task 4 | Complete | Planning fan-out is real outer-graph topology. Six nodes registered by `build_workflow_graph` from one `PlanningNodeFactory.descriptors()` table; `planning_tools` deleted from the parent graph. `planning_agent` resolves to the `planning_model` node via `BASE_AGENT_NODE_OVERRIDES`, honored by both `RoutingInventory.resolve_node` and `resolve_node_for_agent_id`. `dispatch_subagents` is bound as a non-executable schema (`build_dispatch_control_tool`, metadata `non_executable`/`returns_control_command`) and raises `DispatchControlSchemaExecuted` if anything ever runs it. The replay regression passes on both MemorySaver and the production `AsyncPostgresSaver` with the graph *and its connection* discarded between pause and resume: side effects are `w1` then `w1, w2`, never `w1, w1, w2`. 93 focused + 3 PostgreSQL tests pass. |
| 2026-09-03 | Task 4 design notes | Recorded | Two deviations from the plan's literal wording, both forced by LangGraph semantics. (1) `planning_dispatch` and `planning_worker` keep the plan's `-> list[Send]` / `-> dict` signatures as directly-assertable methods, but the *registered* nodes are one-line adapters returning `Command`: a bare `Send` list is only valid from a conditional edge, and the worker's `Command(goto="planning_collect")` is what fans back in (verified: N parallel Sends run the collector exactly once with all results accumulated). (2) Per-task tool narrowing is **not** derived yet — `_planning_allowed_tools` returns `()`, which `WorkerToolScopeMiddleware` now reads as "the agent's own scope" rather than "no tools", because enumerating names needs async tool discovery per task. The restriction a dispatch applies today is the agent identity it chose. |
| 2026-09-04 | Task 5 | Complete | `pending_interrupt_payload(snapshot)` in `hitl_config.py` is the single recovery authority; `_APPROVAL_INTERRUPT_NODES` and `_has_approval_interrupt` are deleted. Wired into the non-stream response, the stream interrupt event, `get_state()`, and resume validation. Provenance is keyed per tool call and rebuilt in `PendingInterruptPayload.to_interrupt_payload()`. Planning nodes emit dispatch validated/collected, worker start/end/interrupt, and worker tool start/end; `V3ProtocolTranslator._translate_custom` projects them onto `subagent_*` events with an allowlisted payload. 144 focused tests pass. |
| 2026-09-04 | Task 5 findings from probing LangGraph 1.2.9 | Recorded | (1) **Task 4's writer events reached nobody.** `astream_events(version="v3")` derives its stream modes from a transformer mux and none of the four native transformers subscribes to `custom`, so every `get_stream_writer()` call was discarded before the iterator. Fixed by passing a `_CustomChannelTransformer` with `required_stream_modes = ("custom",)` via `transformers=`; observed methods went from `{values}` to `{values, custom}`. (2) **A resolved interrupt keeps being reported.** After a partial resume, `snapshot.interrupts` and `task.interrupts` still list the answered interrupt; only `task.result is None` separates a live one. Filtering on truthiness would also re-present a task that returned `{}`. Without this the human would be asked to re-approve a call that already ran. |
| 2026-09-04 | Correction to a recorded claim | Fixed | `test_an_unanswered_interrupt_is_refused_rather_than_left_hanging` asserted that omitting a decision "makes LangGraph replay that node with no answer, which reads as a rejection nobody made". Probed and false: resuming a subset runs exactly the answered tasks, once, and leaves the rest pending. Partial approval is now supported, which is what Task 7 Step 3 ("resolve one exact ID; restart; resolve the other") requires. |
| 2026-09-04 | Task 6 | Complete | Deleted `planning_loop.py`, `rag_loop.py`, `planning_subagents.py`, `_run_agent_in_isolated_context` (525 lines), `_refuse_worker_approval_gated_calls`, and the weak subagent event-sink registry — about 9,400 lines removed. RAG cut over to the shared `RagExecutionGraph` behind one specialist node; `TOOL_STAGE_NODES` is now empty, so neither Planning nor RAG has a parent-level tool stage. `_recover_terminal_response` renamed `_finalized_response` (it stopped salvaging in `051092b`; only the name survived). |
